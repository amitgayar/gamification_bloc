...
class RankingBoardPage extends StatelessWidget {
  const RankingBoardPage({Key? key, required this.board, required this.userId})
      : super(key: key);
  final Board board;
  final String userId;
...
class RankAnimationWidget extends StatelessWidget {
  ...
  animateList(context) async {
    ...
    logPrint.d(
        "animateList called with oldIndex = $oldIndex newIndex = $newIndex _removedPlayer = ${editedPlayer.toJson()}");
...
processBoard(List players, int points, userId) {
  ...
  if (players.isEmpty ||
      players.indexWhere((element) => element.userId == userId) == -1 ||
      points == -1) {
  ...
  for (var i = 0; i < players.length; i++) {
    if (players[i].userId == userId) {
      ...
      _oldIndex = i;
      logPrint.d(
          'processing Board (for loop) -  oldIndex : $_oldIndex - playerEdit = ${_playerEdit.toJson()} ');
    }
  }
  ...
  logPrint.d(
      'processing Board -  oldIndex = $_oldIndex, newIndex = $_newRank, playerEdit = ${_playerEdit.toJson()}');
  return [players, _oldIndex, _newRank, _playerEdit];
}
