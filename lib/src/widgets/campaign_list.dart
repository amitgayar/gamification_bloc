...
class CampaignListWidget extends StatelessWidget {
  const CampaignListWidget({Key? key, required this.onSelection})
      : super(key: key);
  final Function(Campaign) onSelection;

  @override
  Widget build(BuildContext context) {
    dynamic _campaignList =
        context.select((GamificationBloc value) => value.state.campaignList);
    // logPrint.v("_campaignList = $_campaignList");
          ...
          itemBuilder: (context, index) {
            return Container(
              ...
              child: ListTile(
                ...
                subtitle: Text(
                  (_campaignList[index].desc ?? '').toString().toCapitalized(),
